...
class ProcessEK80(ProcessEK):
    ...
    def calc_transmit_signal(self, ed, cal_params):
        ...
        _, unique_signal_idx, counts = np.unique(
            beam_params[np.diag_indices(tau.shape[1], ndim=3)],
            return_index=True,
            return_counts=True,
        )
        ...
        for txi in range(tau.shape[1]):
            ...
            for ch in range(ed.raw.frequency.size):
                ...
                wbt_fil = (
                    ds_fil.attrs[ch_ids[ch] + "_WBT_filter_r"]
                    + 1j * ds_fil.attrs[ch_ids[ch] + "_WBT_filter_i"]
                )
                pc_fil = (
                    ds_fil.attrs[ch_ids[ch] + "_PC_filter_r"]
                    + 1j * ds_fil.attrs[ch_ids[ch] + "_PC_filter_i"]
                )
    ...
    def pulse_compression(self, ed, cal_params, tx_signal):
        ...
        """
        backscatter = (
            ed.raw.backscatter_r + ed.raw.backscatter_i * 1j
        )  # Construct complex backscatter
        ...
        def calc_effective_pulse_length(ping_idx):
            ptxa = np.square(
                np.abs(
                    signal.convolve(
                        tx_signal[ch][ping_idx], np.flipud(tmp_y[ping_idx]), method="direct"
                    )
                    / np.linalg.norm(tx_signal[ch][ping_idx]) ** 2
                )
            )
    ...
    def _cal_broadband(
        self, ed, env_params, cal_params, cal_type, save=True, save_path=None, save_format="zarr"
    ):
        ...
        backscatter_compressed, tau_effective = self.pulse_compression(
            ed, cal_params, tx_signal
        )  # Pulse compress
        ...
        if cal_type == "Sv":
            ...
            Sv["range"] = (
                ...
                self._restructure_range(ed, ranges),
            )
            if save:
    ...
    def calc_range(self, ed, env_params, cal_params, range_bins=None):
        range_bin = (
            xr.DataArray(np.arange(range_bins), coords=[np.arange(range_bins)], dims=["range_bin"])
            if range_bins is not None
            else ed.raw.range_bin
        )
        st = self.calc_sample_thickness(ed, env_params, cal_params)
        range_meter = (
            st * range_bin
            - ed.raw.transmit_duration_nominal * env_params["speed_of_sound_in_water"] / 2
        )  # DataArray [frequency x range_bin]
