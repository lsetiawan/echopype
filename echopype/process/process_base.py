...
class ProcessBase:
    ...
    def calc_sound_speed(self, ed, env_params=None, src="user", formula_source="Mackenzie"):
        ...
        if src == "file":
            with xr.open_dataset(
                ed.raw_path, group="Environment", engine=ed._file_format
            ) as ds_env:
        ...
        if proc_params["MVBS"]["source"] in ["Sv", "Sv_clean"]:
            if getattr(ed, proc_params["MVBS"]["source"]) is not None:
                ...
                if proc_params["MVBS"]["source"] == "Sv":
                    ...
                    raise ValueError(
                        "Sv_clean data has not been found. Please clean Sv data with remove_noise"
                    )
        else:
    ...
    def remove_noise(
        ...
        Sv_noise = (
            noise_est.reindex(
                {"ping_time": power_cal["ping_time"]}, method="ffill"
...
class ProcessEK(ProcessBase):
    ...
    def _cal_narrowband(
        ...
        if cal_type == "Sv":
            # Calc gain
            CSv = (
                10 * np.log10(cal_params["transmit_power"])
            ...
            Sv = (
                ed.raw["backscatter_r"]
                + spreading_loss
                + absorption_loss
                - CSv
                - 2 * cal_params["sa_correction"]
            )
            Sv.name = "Sv"
            Sv = Sv.to_dataset()
            ...
            if save:
                ...
                io.save_file(
                    Sv.chunk({"ping_time": DEFAULT_CHUNK_SIZE["ping_time"]}),
                    Sv_path,
                    mode="w",
                    engine=save_format,
                )
            ...
            if save:
                ...
                io.save_file(
                    Sp.chunk({"ping_time": DEFAULT_CHUNK_SIZE["ping_time"]}),
                    Sp_path,
                    mode="w",
                    engine=save_format,
                )
