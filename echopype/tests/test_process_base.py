...
from ..process import EchoData, ProcessBase
...
def test_validate_proc_path():
    ...
    assert pb.validate_proc_path(ed, postfix="_Sv", save_format="netcdf4") == os.path.join(
        orig_dir, new_filename
    )
...
def test_remove_noise():
    ...
    # Make DataArray
    Sv = xr.DataArray(
        [data], coords=[("frequency", freq), ("ping_time", ping_index), ("range_bin", range_bin)]
    )
    Sv.name = "Sv"
    ds = Sv.to_dataset()
    ed._Sv = ds
    ...
    cal_params = {}
    proc_params = {"noise_est": {"ping_num": 2, "range_bin_num": 5, "SNR": 0}}
...
def test_get_MVBS():
    ...
    Sv = xr.DataArray(
        data_log,
        coords=[("frequency", freq_index), ("ping_time", ping_index), ("range_bin", range_bin)],
    )
    Sv.name = "Sv"
    ds = Sv.to_dataset()
    ed._Sv = ds
    ...
    shape = (
        nfreq,
        npings / proc_params["MVBS"]["ping_num"],
        nrange / proc_params["MVBS"]["range_bin_num"],
    )
    ...
    assert np.allclose(
        roll_avg_range,
        data_test.dropna("ping_time", "all").isel(frequency=0, ping_time=0),
        equal_nan=True,
    )
    ...
    assert np.allclose(
        roll_avg_ping,
        data_test.dropna("range_bin", "all").isel(frequency=0, range_bin=0),
        equal_nan=True,
    )
