...
raw_path_bb_cw = (
    "./echopype/test_data/ek80/Summer2018--D20180905-T033113.raw"  # Large file (CW and BB)
)
...
power_test_path = [
    "./echopype/test_data/ek80/from_echoview/18kHz.power.csv",
...
def test_cw():
    ...
    tmp = Convert()
    tmp.set_source(file=raw_path_cw, model="EK80")
    tmp.to_netcdf()
    ...
    with xr.open_dataset(tmp.output_file, group="Beam") as ds_beam:
        ...
        major = (
            ds_beam["angle_athwartship"] * 1.40625 / ds_beam["angle_sensitivity_athwartship"]
            - ds_beam["angle_offset_athwartship"]
        )[:, 1:4, :]
        minor = (
            ds_beam["angle_alongship"] * 1.40625 / ds_beam["angle_sensitivity_alongship"]
        ...
        for f in np.unique(df["frequency"]):
            ...
            for i in np.unique(df_freq["ping_index"]):
                ...
                major_test.append(
                    xr.DataArray(val_maj, coords=[("range_bin", np.arange(val_maj.size))])
                )
                minor_test.append(
                    xr.DataArray(val_min, coords=[("range_bin", np.arange(val_min.size))])
                )
            assert np.allclose(
                xr.concat(major_test, "ping_time").dropna("range_bin"),
                major.sel(frequency=f).dropna("range_bin"),
            )
            assert np.allclose(
                xr.concat(minor_test, "ping_time").dropna("range_bin"),
...
def test_add_platform():
    ...
    location_time = pd.date_range(
        start="2019-08-22T16:00:00.0", end="2019-08-22T16:15:00.0", periods=100
    )
    lat = np.random.rand(100)
    lon = np.random.rand(100)
    testing_ds = xr.Dataset(
        {"lat": (["location_time"], lat), "lon": (["location_time"], lon)},
