...
csv_paths = [
    "./echopype/test_data/ek60/from_echoview/DY1801_EK60-D20180211-T164025-Power18.csv",
...
def test_convert_matlab():
    ...
    with xr.open_dataset(test_path) as ds_test:
        assert np.allclose(
            ds_test.power, ds_beam.backscatter_r
        )  # Identical to MATLAB output to 1e-6
        athwartship = (
...
def test_combine():
    ...
    ####################
    # NOTE: There should be raw_paths defined below,
    # the blank list is just a placeholder and this test will fail if not skipped!
    raw_paths = []
    ######################
...
def test_save_to_s3():
    ...
    raw_paths = [
        "./echopype/test_data/ek60/set1/Winter2017-D20170115-T131932.raw",
...
def test_save_to_gcloud():
    ...
    fs = gcsfs.GCSFileSystem(
        token='C:/Users/"user"/AppData/Roaming/gcloud/application_default_credentials.json'
    )
...
def test_save_to_azure():
    ...
    )  # Save azure keys in environment variable first
