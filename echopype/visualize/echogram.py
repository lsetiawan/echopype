...
class EchoGram:
    ...
    def plot(
        self, form, frequency=None, plot_ping_number=False, infer_burst=False, col_wrap=2, **kwargs
    ):
        """ Plots an echogram
        ...
        **kwargs optional
        ...
        if form == "MVBS":
            ...
            if not frequency or (
                isinstance(frequency, set) and all(isinstance(f, int) for f in frequency)
            ):
                if isinstance(frequency, set):
                    dropped_freqs = list(
                        set(getattr(self.echo_data, form).frequency.values) - frequency
                    )
                    to_plot = data.MVBS.drop(dropped_freqs, dim="frequency")
                else:
                    to_plot = data.MVBS
                to_plot.plot(
                    col="frequency", col_wrap=col_wrap, y="range_bin", **kwargs
                ).set_xlabels("Ping time").set_ylabels("Range bin")
                return
            else:
        ...
        if plot_ping_number:
            ping_num = xr.DataArray(
                np.arange(len(data.ping_time)), coords=[data.ping_time], dims=["ping_time"]
            )
            ...
            if data.ndim == 2:
                ...
                data.plot(
                    col="frequency", col_wrap=col_wrap, x="ping_num", y="depth", **kwargs
                ).set_xlabels("Ping number").set_ylabels("Depth (m)")
                return
        # Plot ping time on x axis
            ...
            if not infer_burst:
                if plot_ping_number:
                    raise ValueError(
                        "Plotting with ping number cannot be done unless infer_burst = True"
                    )
                ...
                if ping_per_profile != 1:
                    ...
                    while ping < total_pings:
                        pt = np.insert(
                            pt,
                            ping + count,
                            pt[ping - 1 + count] + np.timedelta64(ping_period, "s"),
                        )
                    ...
                    if data.ndim == 2:
                        to_plot = xr.DataArray(
                            var,
                            coords={"ping_time": pt, "depth": data.depth},
                        ...
                        to_plot.plot(
                            ...
                            y="depth",
                            **kwargs
                        ).set_xlabels("Ping time").set_ylabels("Depth (m)")
                        return
            ...
            # Default interpolation for intervals between bursts
                ...
                if data.ndim == 2:
                    ...
                    data.plot(
                        col="frequency", col_wrap=col_wrap, x="ping_time", y="depth", **kwargs
                    ).set_xlabels("Ping time").set_ylabels("Depth (m)")
                    return
