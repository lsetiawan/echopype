...
class SetGroupsEK80(SetGroupsBase):
    ...
    def save(self):
        ...
        if "ENV" in self.parser_obj.data_type:
            self.set_toplevel(
                self.sonar_model, date_created=self.parser_obj.environment["timestamp"]
            )
            self.set_provenance()
        ...
        # Save all groups
        self.set_toplevel(
            self.sonar_model, date_created=self.parser_obj.config_datagram["timestamp"]
        )
    ...
    def set_env(self, env_only=False):
        ...
        ping_time = np.array(
            [
                (ping_time.astype("datetime64[ns]") - np.datetime64("1900-01-01T00:00:00"))
                / np.timedelta64(1, "s")
            ]
        )
        ...
        ds = xr.Dataset(
            {
                "temperature": (["ping_time"], [self.parser_obj.environment["temperature"]]),
    ...
    def set_platform(self):
        ...
        mru_time = (
            (np.array(mru_time) - np.datetime64("1900-01-01T00:00:00")) / np.timedelta64(1, "s")
            if mru_time is not None
            else [np.nan]
        )
            ...
            ds_f_start_end = xr.Dataset(
                {
                    "frequency_start": (
                        ["ping_time"],
                        np.array(self.parser_obj.ping_data_dict["frequency_start"][ch], dtype=int),
                        {"long_name": "Starting frequency of the transducer", "units": "Hz"},
                    ),
                    "frequency_end": (
                        ["ping_time"],
                        np.array(self.parser_obj.ping_data_dict["frequency_end"][ch], dtype=int),
                        {"long_name": "Ending frequency of the transducer", "units": "Hz"},
                    ),
                },
                coords={
                    "ping_time": (
        ...
        ds_common = xr.Dataset(
            {
                ...
                "transmit_power": (
                    ...
                    {"long_name": "Nominal transmit power", "units": "W", "valid_min": 0.0},
                ),
            ...
            ds_combine = ds_combine.assign_coords(
                {
                    "ping_time": (
                        ["ping_time"],
                        (ds_combine["ping_time"] - np.datetime64("1900-01-01T00:00:00"))
                        / np.timedelta64(1, "s"),
