...
class SimradDepthParser(_SimradDatagramParser):
    ...
    def __init__(self):
        headers = {
            0: [("type", "4s"), ("low_date", "L"), ("high_date", "L"), ("transceiver_count", "L")]
        }
        ...
        if version == 0:
            ...
            for indx in range(data["transceiver_count"]):
                datagram_contents.extend(
                    [data["depth"][indx], data["reflectivity"][indx], data["unused"][indx]]
                )
    ...
    def _unpack_contents(self, raw_string, bytes_read, version):
        ...
        if version == 0:
            ...
            data["depth"] = np.fromiter(
                struct.unpack(depth_fmt, raw_string[buf_indx : buf_indx + depth_size]), "float"
            )
            ...
            if data["subtype"] == "configuration":
                ...
                for tcvr in root.iter("Transceiver"):
                    ...
                    for tcvr_ch in tcvr.iter("Channel"):
                        ...
                        h = root.find("Header")
                        dict_to_dict(
                            h.attrib, data["configuration"][channel_id], self.header_parsing_options
                        )
                ...
                for h in root.iter("Transducer"):
                    ...
                    #  add the data to the environment dict
                    dict_to_dict(transducer_xml, data["environment"], self.envxdcr_parsing_options)
        ...
        if version == 0:
            ...
            for txcvr_indx in range(1, data["transceiver_count"] + 1):
                txcvr_header_values_encoded = struct.unpack(
                    txcvr_header_fmt, raw_string[buf_indx : buf_indx + txcvr_header_size]
                )
            ...
            if data["count"] > 0:
                ...
                if data["n_complex"] > 0:
                    ...
                    data["complex"] = np.frombuffer(
                        raw_string[indx : indx + block_size], dtype=data["complex_dtype"]
                    )
    ...
    def _pack_contents(self, data, version):
        ...
        if version == 0:
            ...
            if data["count"] > 0:
                ...
                if data["mode"] == 0:
                    log.warning(
                        "Data 'count' = %d, but mode == 0.  Setting count to 0", data["count"]
                    )
                    data["count"] = 0
