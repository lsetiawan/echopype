...
class SetGroupsAZFP(SetGroupsBase):
    ...
    def set_env(self):
        ...
        ping_time = (
            self.parser_obj.ping_time - np.datetime64("1970-01-01T00:00:00")
        ) / np.timedelta64(1, "s")
        ds = xr.Dataset(
            {"temperature": (["ping_time"], self.parser_obj.unpacked_data["temperature"])},
            coords={
                "ping_time": (
                    ["ping_time"],
                    ping_time,
    ...
    def set_beam(self):
        ...
        ping_time = (
            self.parser_obj.ping_time - np.datetime64("1900-01-01T00:00:00")
        ) / np.timedelta64(1, "s")
