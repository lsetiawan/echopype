...
def perform_combination(sonar_model, input_paths, output_path, engine):
    ...
    def coerce_type(ds, group):
        if group == "Beam":
            if sonar_model == "EK80":
                ds["transceiver_software_version"] = ds["transceiver_software_version"].astype(
                    "<U10"
                )
    ...
    with xr.open_mfdataset(
        input_paths, group="Beam", concat_dim="ping_time", data_vars="minimal", engine=engine
    ) as ds_beam:
    ...
    with xr.open_mfdataset(
        input_paths, group="Environment", concat_dim="ping_time", data_vars="minimal", engine=engine
    ) as ds_env:
        io.save_file(
    ...
    if sonar_model in ["EK60", "EK80", "EA640"]:
        with xr.open_mfdataset(
            ...
            io.save_file(
                ds_nmea.chunk({"location_time": DEFAULT_CHUNK_SIZE["ping_time"]}).astype("str"),
                path=output_path,
                mode="a",
                engine=engine,
                group="Platform/NMEA",
